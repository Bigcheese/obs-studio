...
static obs_properties_t *chroma_key_properties(void *data)
{
	...
	obs_properties_add_int(props, SETTING_SIMILARITY, TEXT_SIMILARITY,
			1, 1000, 1);
	obs_properties_add_int(props, SETTING_SMOOTHNESS, TEXT_SMOOTHNESS,
			1, 1000, 1);
	obs_properties_add_int(props, SETTING_SPILL, TEXT_SPILL, 1, 1000, 1);

	obs_properties_add_int(props, SETTING_OPACITY, TEXT_OPACITY, 0, 100, 1);
	obs_properties_add_float(props, SETTING_CONTRAST, TEXT_CONTRAST,
			-1.0, 1.0, 0.01);
	obs_properties_add_float(props, SETTING_BRIGHTNESS, TEXT_BRIGHTNESS,
			-1.0, 1.0, 0.01);
	obs_properties_add_float(props, SETTING_GAMMA, TEXT_GAMMA,
			-1.0, 1.0, 0.01);
...
static void chroma_key_defaults(obs_data_t *settings)
{
	...
	obs_data_set_default_int(settings, SETTING_SIMILARITY, 1000);
	obs_data_set_default_int(settings, SETTING_SMOOTHNESS, 80);
	obs_data_set_default_int(settings, SETTING_SPILL, 50);
}
